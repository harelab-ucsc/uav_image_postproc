...
- (`0 = ignore' , '255 = keep`)
- `python gen_leg_masks.py --overlay-dir masks_overlays` - detect dorne legs in cam2 & cam4 and write one canonical mask per camera, plus overlays sheets to visually check the masks.
- `python gen_leg_masks.py --materialize` - link that canonical mask to every frame
